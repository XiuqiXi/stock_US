...
def download_data(config):
    ...
    data, meta_data = ts.get_intraday(config["alpha_vantage"]["symbol"], \
                                      interval = config["alpha_vantage"]["interval"], \
                                          outputsize=config["alpha_vantage"]["outputsize"])
    # data, meta_data = ts.get_daily(config["alpha_vantage"]["symbol"], \
    #                                outputsize=config["alpha_vantage"]["outputsize"])
...
def run_epoch(dataloader, is_training=False, model = model, optimizer = optimizer, scheduler = scheduler, criterion = criterion):
 	epoch_loss = 0
